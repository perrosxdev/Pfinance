...
import { pruneTransactionsToLastNMonths } from '../utils/retention';
  ...
  return (
    <SafeAreaView style={gstyles.container}>
      <View style={gstyles.headerCard}>
        <Text style={gstyles.headerText}>Total Gastos</Text>
      ...
      <View style={gstyles.actions}>
        <TouchableOpacity style={gstyles.primaryButton} onPress={() => { setEditing(null); setModalVisible(true); }}>
          <IconButton name="add" color="#fff" size={18} label="Nuevo gasto" labelStyle={{ color: '#fff', fontWeight: '700' }} />
        </TouchableOpacity>
      </View>
      ...
      <ScrollView horizontal contentContainerStyle={{ minWidth: 700 }}>
        <View style={gstyles.tableWrapper}>
          ...
          <FlatList
            ...
            ListEmptyComponent={<View style={{ padding: 20 }}><Text>No hay gastos registrados.</Text></View>}
          />
        </View>
      </ScrollView>

      <TransactionForm visible={modalVisible} onClose={() => { setModalVisible(false); setEditing(null); }} onSubmit={handleSubmit} initialData={editing} title={editing ? 'Editar gasto' : 'Nuevo gasto'} />
    </SafeAreaView>
  );
}
